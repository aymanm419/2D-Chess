...
char* imagePath(std::string s,std::string fileName)
{
    std::string tmp = s + fileName;
    char char_array[tmp.length()];
...
int main( int numArgs, char *args[])
{
    ...
    // get '/' or '\\' depending on unix/mac or windows.
        ...
        int pos = aux.rfind('/');
    #endif
    std::string path = aux.substr(0,pos+1) + "Sprites\\";
    std::string tmp = path + "chessBack.bmp";
    ...
    while(!Game.quit && Game.mainEvent->type != SDL_QUIT)
    {
        ...
        if((GameHandler.turn == PIECE_COLOR::BLACK && GameHandler.isAI) || (!GameHandler.isAI))
        {
            ...
            GameHandler.getNextMove(Game,0,PIECE_COLOR::WHITE);
            GameHandler.ChoosePiece(Game,GameHandler.lastChoosenPiece);
            GameHandler.MovePiece(Game);
